...
Model = this.Model = Class({
  ...
  save: function(fn){
    var self = this;
    if(this.isNew){
      this._collection.save(this._normalize(), function(err,rec){
        self.isNew = false;
        if(fn) fn(err,rec);
      });
    } else {
      this._collection.update({_id: this._id},{$set: this._getDirty()},function(err,recs){
        self._dirty = {};
        if(fn) fn(err,recs);
      });
    }
  ...
  extend: {
    select: function(fields, hydrate){
      var _fields = fields || {}, self = this,
	  thisPromise = null,
	  resCallback = function(err, cursor){
          ...
          cursor.each(function(err, doc){
            ...
            if(doc != null){
              ...
              results.push((hydrate === true) ? new self(doc, true) : doc);
            } else {
              thisPromise.complete(results);
            }
          });
        },
      _writer = new query.Writer(function(promise){
	  	thisPromise = promise;
	  	if (this.isCMD) { // a promise method has been called. Prep...
	  		var opts = {};
			if (this.query)
				opts.query = this.query;
			self._collection.mapReduce(this.map, this.reduce, opts, function(err, collection) { 
				collection.find({},resCallback);
			})
		} else {
	        self._collection.find(this.query, this.options, resCallback);
		}
      });
      for (var i in _fields) _writer.select(i, _fields[i]);
      return _writer;
    },
    ...
    find: function(where, hydrate){
      ...
      _writer = new query.Writer(function(promise){
        self._collection.find(this.query, this.options, function(err, cursor){
          if (err) return self._connection._error(err);
          var results = [];
          cursor.each(function(err, doc){
            if (err) return self._connection._error(err);

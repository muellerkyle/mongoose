...
var aggragates = {
		$sum: function(g,vals){
			...
			for (var i=0;i<vals.length;i++)
				res.val+=(isNaN(vals[i].val) ? 0 : vals[i].val);
			return res;
		},
    ...
    if (arguments.length == 1 && typeof arguments[0] == 'object'){
		...
		if (/^\$/.test(arguments[0])){
			...
			if (typeof(this._aggRdc) == 'function') { // Set map function
				if (arguments[1] && arguments[1].field){ // Only support of top level props for now
					var grpMeta = ((typeof arguments[1].group == 'string') ? 'this["'+arguments[1].group+'"]' : '"all"');
					this._aggMap = new Function('emit('+grpMeta+', {val:this["'+arguments[1].field+'"]})');
				}	
			}
		}
		else
